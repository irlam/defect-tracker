...
class Navbar {
    ...
    public function render() {
        ...
        foreach ($navbarItems as $item) {
            ...
            echo '<li class="nav-item' . ($isDropdown ? ' dropdown' : '') . '">';
            if ($isDropdown) {
                ...
                echo '</a>';
                echo '<div class="dropdown-menu dropdown-menu-dark shadow" aria-labelledby="' . htmlspecialchars($item['id'], ENT_QUOTES, 'UTF-8') . '">';
                foreach ($item['dropdown'] as $dropdownItem) {
                    if (($dropdownItem['label'] ?? '') === '---divider---') {
                        echo '<div class="dropdown-divider"></div>';
                        continue;
                    }
                    if (($dropdownItem['type'] ?? '') === 'header') {
                        echo '<h6 class="dropdown-header text-uppercase text-muted small">' . htmlspecialchars($dropdownItem['label'], ENT_QUOTES, 'UTF-8') . '</h6>';
                        continue;
                    }
                    echo '<a class="dropdown-item" href="' . htmlspecialchars($dropdownItem['url'], ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($dropdownItem['label'], ENT_QUOTES, 'UTF-8') . '</a>';
                }
                echo '</div>';
            } else {
